...
public class ClassParser{
	...
	static ArrayList<SearchObject> parse(String path, List<String> content){
		...
		for(int i = 0; i < content.size(); i++){
			if((content.get(i).contains("class ") ||
			    content.get(i).contains("interface")) &&
			   realClass(content.get(i))){
				ClassObject classObject =
						parseClassDecLine(getDeclarationLine(content, i));
	...
	private static ClassObject parseClassDecLine(String declarationLine){
		ClassObject classObject =
				new ClassObject(null, null, SearchObject.AccessModifier.DEFAULT,
				                0, null);
		...
		if(declarationLine.contains("interface"))
			...
			classObject.setClassType(ClassObject.ClassType.DEFAULT);
		
		if(declarationLine.contains("abstract"))
			classObject
					.setInheritanceType(ClassObject.InheritanceType.ABSTRACT);
		else if(declarationLine.contains("final"))
			classObject.setInheritanceType(ClassObject.InheritanceType.FINAL);
		else
			classObject.setInheritanceType(ClassObject.InheritanceType.DEFAULT);
		...
		SearchObject.AccessModifier accessModifier =
				visibilityParser(declarationLine);
		classObject.setAccessModifier(accessModifier);
		
		for(int i = 0; i < lineArgs.length; i++){
			if(lineArgs[i].contains("class") ||
			   lineArgs[i].contains("interface")){
				if(lineArgs[i + 1].contains("<") &&
				   lineArgs[i + 1].contains(">")){
					classObject.setHasGeneric(lineArgs[i + 1].contains("<") &&
					                          lineArgs[i + 1].contains(">"));
					classObject.setName(lineArgs[i + 1].substring(0,
					                                              lineArgs[i +
					                                                       1]
							                                              .indexOf(
									                                              "<")));
	...
	public static FieldObject fieldParser(String fieldString, String path){
		...
		return new FieldObject(stream[stream.length - 1], visibility, path, 0,
	...
	public static MethodObject methodParser(String method, String path){
		...
		String[]                    front      =
				method.substring(0, method.indexOf('(')).split(" ");
		String[]                    back       =
				method.substring(method.indexOf('(') + 1, method.indexOf(')'))
				      .split(", ");
		MethodObject                result     = null;
		
		result = new MethodObject(front[front.length - 1], visibility, path, 0,
		                          null, method.contains("static"),
		                          front[front.length - 2]);
